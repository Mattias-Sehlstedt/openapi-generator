...
class PetApiController() {
    ...
    fun findPetsByStatus(@NotNull @Parameter(description = "Status values that need to be considered for filter", required = true, array = ArraySchema(schema = Schema(allowableValues = ["available", "pending", "sold"]))) @Valid @RequestParam(value = "status", required = true) status: kotlin.collections.List<kotlin.String>): ResponseEntity<List<Pet>> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
